...
def merge(A1, A2):
    num_states = A1.shape[0]
    n_s_1 = A1.shape[2]
    n_s_2 = A2.shape[2]

    n_s_merge = n_s_1 + n_s_2 - 1
    A_merge = np.zeros(shape=(num_states, num_states, n_s_merge))
    for s_b in np.arange(0, num_states):
        for s_e in np.arange(0, num_states):
            for i in np.arange(0, num_states):
                A_merge[s_b, s_e, :] += np.convolve(A1[s_b, i, :], A2[i, s_e, :])
...
# 1. The GPU Kernel
@cuda.jit
def merge_numba_kernel(A1, A2, A_merge):
    # Calculate the 3D index for this specific thread
    sb, se, k = cuda.grid(3)

    # Boundary check (threads might exceed array size)
    if sb < A_merge.shape[0] and se < A_merge.shape[1] and k < A_merge.shape[2]:
        num_states = A1.shape[1]
        n_s_1 = A1.shape[2]
        n_s_2 = A2.shape[2]

        tmp_sum = 0.0
        for i in range(num_states):
            for j in range(n_s_1):
                k_minus_j = k - j
                if 0 <= k_minus_j < n_s_2:
                    tmp_sum += A1[sb, i, j] * A2[i, se, k_minus_j]

        A_merge[sb, se, k] = tmp_sum


def run_numba_merge(A1, A2):
    n_s_merge = A1.shape[2] + A2.shape[2] - 1
    out_shape = (A1.shape[0], A2.shape[1], n_s_merge)

    # Transfer data to the GPU
    d_A1 = cuda.to_device(A1.astype(np.float64))
    d_A2 = cuda.to_device(A2.astype(np.float64))
    d_out = cuda.device_array(out_shape, dtype=np.float64)

    # Define block and grid dimensions
    # 8x8x8 = 512 threads per block
    threads_per_block = (8, 8, 8)
    blocks_per_grid_x = math.ceil(out_shape[0] / threads_per_block[0])
    blocks_per_grid_y = math.ceil(out_shape[1] / threads_per_block[1])
    blocks_per_grid_z = math.ceil(out_shape[2] / threads_per_block[2])
    blocks_per_grid = (blocks_per_grid_x, blocks_per_grid_y, blocks_per_grid_z)

    # Launch kernel
    merge_numba_kernel[blocks_per_grid, threads_per_block](d_A1, d_A2, d_out)
...
def main():
    ...
    print("A_12 finished")
    A_15 = run_numba_merge(A_12, A_3)
    print("A_15 finished")

    result = np.zeros(shape=(2 * num_trellis_stages + 1))
    for i in np.arange(0, num_total_states, 2**m):
        result += A_15[i, i, :]
